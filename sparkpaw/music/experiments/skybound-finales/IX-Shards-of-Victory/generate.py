...
LEAD=add('IX-Shards-of-Victory LEAD',echo(lead,.20),47)
...
patterns=ROWS//64
header=b'IX-SHARDS-OF-VICTORY'.ljust(20,b'\0')
...
(OUT/'IX-Shards-of-Victory.mod').write_bytes(module)
...
with wave.open(str(OUT/'IX-Shards-of-Victory-preview.wav'),'wb') as w:
    w.setparams((2,2,44100,0,'NONE','not compressed'));w.writeframes(np.rint(x).astype('<i2').tobytes())
report={'title':'Sparkpaw — IX-Shards-of-Victory','status':'original intro finale candidate; user listening pending','bpm':BPM,'scene_tempos':TEMPOS,'scene_start_seconds':scene_times,'speed':SPEED,'channels':4,'patterns':patterns,'bars':BARS,'render_seconds':seconds,'module_bytes':len(module),'sample_bytes':sum(len(d) for _,d,_ in samples),'module_sha256':hashlib.sha256(module).hexdigest(),'peak_before_master':int(abs(pcm.astype(np.int32)).max()),'preview_gain':gain,'reference_samples_used':False,'runtime_integrated':False,'revision':'IX-Shards-of-Victory: accepted opening preserved; continuous 164 BPM hopeful finale','render_engine':'micromod, actual MOD playback','samples':[{'name':n,'bytes':len(d),'volume':v} for n,d,v in samples]}
