"""Original Sparkpaw title composition: four-channel ProTracker MOD + real MOD preview.
...
t=time(.48);p=TAU*freq(36)*t
bass=np.sin(p)+.44*np.sin(2*p)*np.exp(-t*5)+.2*np.sin(3*p)*np.exp(-t*11)+.08*np.sin(5*p)*np.exp(-t*20)
BASS=add('SHARD PULSE BASS',np.tanh(1.4*bass)*(1-np.exp(-t*1400))*np.exp(-t*6),57)
# Lead: detuned harmonic brass with a brief metallic attack and decaying edge.
t=time(1.5);f=freq(48);lead=np.zeros_like(t)
for det in [-.0014,.0014]:
    phase=TAU*f*(1+det)*t+.018*np.sin(TAU*5.6*t)*np.minimum(t/.22,1)
    for h,a in [(1,1),(2,.40),(3,.26),(4,.10),(5,.07),(7,.025)]:
        lead+=a*np.sin(h*phase+.12*h)*np.exp(-t*(.8+.6*h))
lead*=np.minimum(t/.006,1)
LEAD=add('STORMBRASS HERO',echo(lead,.20),47)
...
ARP=add('CLOCKWORK PLUCK',(np.sin(p)+.42*np.sin(2*p)+.2*np.sin(3*p))*np.exp(-t*19),32)
...
for bar,chord in enumerate(progression):
    ...
    root=roots[chord]
    for j,step in enumerate([0,6,10,14] if intro else [0,2,3,6,8,10,11,14]):
        pitch=root+(12 if step in (6,14) else 0)
        if step==14 and bar in [3,11,19,27,35,43,47]: pitch=roots[progression[(bar+1)%48]]-1
        note(bar,step,0,BASS,pitch+12,54 if j%3==0 else 43)
    if bridge:
        tones={'Am':[57,60,64,69],'Em':[55,59,64,67],'C':[55,60,64,67],'D':[57,62,66,69],'B7':[57,59,63,66]}[chord]
        for j,step in enumerate(range(0,16,2)):note(bar,step,2,ARP,tones[[0,2,1,3,2,1,3,2][j]],27 if j%2 else 34)
    else:
        for step,vol in [(0,34),(6,22),(8,29),(14,24)]:note(bar,step,2,CHORD[chord],48,vol if not intro else vol-5)
    drums={0:(KICK,48,61),2:(HAT,48,25),4:(SNARE,48,55),6:(HAT,48,22),8:(KICK,48,58),10:(OPEN,48,24),12:(SNARE,48,55),14:(HAT,48,27)}
    if bar%2:drums[11]=(KICK,48,45)
    if bar in [0,4,12,28,36,44]:drums[0]=(CRASH,48,55)
    if bar in [3,11,19,27,35,43,47]:
    ...
    if bridge and bar<24:drums={0:(KICK,48,54),4:(HAT,48,24),8:(SNARE,48,45),12:(OPEN,48,24),14:(KICK,48,46)}
...
A=[[(0,64,3),(3,67,2),(6,71,4),(12,69,2),(14,67,2)],
 [(0,64,5),(6,67,2),(8,69,3),(12,67,2),(14,64,2)],
 [(0,62,3),(4,67,3),(8,71,5),(14,69,2)],
 [(0,66,5),(6,64,2),(8,62,5)],
 [(0,64,2),(3,67,2),(6,71,3),(10,69,2),(12,67,3)],
 [(0,69,3),(4,67,2),(7,64,3),(12,60,3)],
 [(0,64,3),(4,69,5),(10,67,2),(13,64,2)],
 [(0,63,5),(6,66,2),(9,69,3),(13,66,2),(15,63,1)]]
B=[[(0,67,5),(6,64,2),(9,67,3),(13,69,2)],
 [(0,69,3),(4,66,3),(8,62,3),(12,66,3)],
 [(0,71,6),(8,67,3),(12,64,3)],
 [(0,67,3),(4,62,3),(8,59,3),(12,62,3)],
 [(0,69,5),(6,67,2),(8,64,3),(12,60,3)],
 [(0,67,3),(4,69,3),(8,71,3),(12,67,3)],
 [(0,69,3),(4,66,2),(7,64,3),(12,62,3)],
 [(0,63,5),(6,66,2),(9,69,3),(13,71,2)]]
intro=[[(0,64,5),(8,71,4)],[(2,67,4),(10,64,4)],[(0,66,5),(8,69,4)],[(2,63,3),(8,66,3),(12,71,3)]]
bridge=[[(0,69,6),(10,64,4)],[(2,67,5),(10,64,4)],[(0,67,6),(9,64,4)],[(2,66,5),(10,63,4)],[(0,64,3),(4,69,3),(10,67,4)],[(0,67,5),(8,64,2),(12,67,3)],[(0,66,3),(4,69,3),(8,66,3),(12,62,3)],[(0,63,3),(4,66,3),(8,69,3),(12,71,3)]]
last=[[(0,67,5),(8,64,4)],[(0,69,5),(8,66,4)],[(0,71,5),(8,69,2),(12,66,2)],[(0,63,6),(8,66,2),(12,63,2)]]
...
for bar,phrase in enumerate(phrases):
    ...
    for step,m,duration in phrase:
        ...
        end=bar*16+step+duration
        if end<ROWS and end%16 and all(n[0]!=end%16 for n in phrase):put(end,3,fx=12,arg=0)
        if duration>=5:put(bar*16+step+2,3,fx=4,arg=0x12)
...
patterns=ROWS//64
header=b'SPARKPAW STORMBOUND'.ljust(20,b'\0')
...
(OUT/'Stormbound.mod').write_bytes(module)
...
with wave.open(str(OUT/'Stormbound-preview.wav'),'wb') as w:
    w.setparams((2,2,44100,0,'NONE','not compressed'));w.writeframes(np.rint(x).astype('<i2').tobytes())
report={'title':'Sparkpaw — Stormbound','status':'original title candidate; user listening pending','bpm':BPM,'speed':SPEED,'channels':4,'patterns':patterns,'bars':BARS,'render_seconds':seconds,'module_bytes':len(module),'sample_bytes':sum(len(d) for _,d,_ in samples),'module_sha256':hashlib.sha256(module).hexdigest(),'peak_before_master':int(abs(pcm.astype(np.int32)).max()),'preview_gain':gain,'reference_samples_used':False,'runtime_integrated':False,'render_engine':'micromod, actual MOD playback','samples':[{'name':n,'bytes':len(d),'volume':v} for n,d,v in samples]}
