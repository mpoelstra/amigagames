...
for name,notes in voicings.items():
    ...
    x*=np.minimum(t/.035,1)*np.exp(-t*1.4)
...
for bar,chord in enumerate(progression):
    ...
    for j,step in enumerate([0,8] if intro else [0,2,4,6,8,10,12,14]):
        ...
        note(bar,step,0,BASS,pitch+12,51 if j%4==0 else 44)
    if bridge:
        ...
        for step,vol in [(0,30),(8,27)]:note(bar,step,2,CHORD[chord],48,vol if not intro else vol-5)
...
patterns=ROWS//64
header=b'STORMBOUND SMOOTH'.ljust(20,b'\0')
...
(OUT/'Stormbound-Smooth.mod').write_bytes(module)
...
with wave.open(str(OUT/'Stormbound-Smooth-preview.wav'),'wb') as w:
    w.setparams((2,2,44100,0,'NONE','not compressed'));w.writeframes(np.rint(x).astype('<i2').tobytes())
report={'title':'Sparkpaw — Stormbound Smooth','status':'original title candidate; user listening pending','bpm':BPM,'speed':SPEED,'channels':4,'patterns':patterns,'bars':BARS,'render_seconds':seconds,'module_bytes':len(module),'sample_bytes':sum(len(d) for _,d,_ in samples),'module_sha256':hashlib.sha256(module).hexdigest(),'peak_before_master':int(abs(pcm.astype(np.int32)).max()),'preview_gain':gain,'reference_samples_used':False,'runtime_integrated':False,'revision':'v2: connected melody, even eighth-note bass, regular backbeat, sustained chords, no lead note cuts','render_engine':'micromod, actual MOD playback','samples':[{'name':n,'bytes':len(d),'volume':v} for n,d,v in samples]}
