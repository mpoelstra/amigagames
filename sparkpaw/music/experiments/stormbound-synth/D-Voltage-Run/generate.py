...
LEAD=add('D-Voltage-Run LEAD',echo(lead,.20),47)
...
patterns=ROWS//64
header=b'D-VOLTAGE-RUN'.ljust(20,b'\0')
...
(OUT/'D-Voltage-Run.mod').write_bytes(module)
...
with wave.open(str(OUT/'D-Voltage-Run-preview.wav'),'wb') as w:
    w.setparams((2,2,44100,0,'NONE','not compressed'));w.writeframes(np.rint(x).astype('<i2').tobytes())
report={'title':'Sparkpaw — D-Voltage-Run','status':'original title candidate; user listening pending','bpm':BPM,'speed':SPEED,'channels':4,'patterns':patterns,'bars':BARS,'render_seconds':seconds,'module_bytes':len(module),'sample_bytes':sum(len(d) for _,d,_ in samples),'module_sha256':hashlib.sha256(module).hexdigest(),'peak_before_master':int(abs(pcm.astype(np.int32)).max()),'preview_gain':gain,'reference_samples_used':False,'runtime_integrated':False,'revision':'D-Voltage-Run: extended original melody and four-channel arrangement','render_engine':'micromod, actual MOD playback','samples':[{'name':n,'bytes':len(d),'volume':v} for n,d,v in samples]}
