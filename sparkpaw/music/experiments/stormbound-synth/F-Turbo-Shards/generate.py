...
LEAD=add('F-Turbo-Shards LEAD',echo(lead,.20),47)
...
ARP=add('CLOCKWORK PLUCK',(np.sin(p)+.30*np.sin(2*p)+.32*np.sin(3*p)+.12*np.sin(5*p))*np.exp(-t*7),32)
...
for bar,chord in enumerate(progression):
    ...
    if bridge or (bar>=4 and bar%4 in (1,3)):
        ...
        for step,vol in [(0,25),(2,32),(8,25),(10,30)]:note(bar,step,2,CHORD[chord],48,vol if not intro else vol-5)
...
patterns=ROWS//64
header=b'F-TURBO-SHARDS'.ljust(20,b'\0')
...
(OUT/'F-Turbo-Shards.mod').write_bytes(module)
...
with wave.open(str(OUT/'F-Turbo-Shards-preview.wav'),'wb') as w:
    w.setparams((2,2,44100,0,'NONE','not compressed'));w.writeframes(np.rint(x).astype('<i2').tobytes())
report={'title':'Sparkpaw — F-Turbo-Shards','status':'original title candidate; user listening pending','bpm':BPM,'speed':SPEED,'channels':4,'patterns':patterns,'bars':BARS,'render_seconds':seconds,'module_bytes':len(module),'sample_bytes':sum(len(d) for _,d,_ in samples),'module_sha256':hashlib.sha256(module).hexdigest(),'peak_before_master':int(abs(pcm.astype(np.int32)).max()),'preview_gain':gain,'reference_samples_used':False,'runtime_integrated':False,'revision':'F-Turbo-Shards: extended original melody and four-channel arrangement','render_engine':'micromod, actual MOD playback','samples':[{'name':n,'bytes':len(d),'volume':v} for n,d,v in samples]}
