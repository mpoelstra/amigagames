...
for det in [-.0014,.0014]:
    ...
    for h,a in [(1,1),(2,.32),(3,.30),(4,.08),(5,.12),(7,.045)]:
        lead+=a*np.sin(h*phase+.12*h)*np.exp(-t*(.12+.11*h))
lead*=np.minimum(t/.015,1)
LEAD=add('B-Skyward LEAD',echo(lead,.20),47)
...
for bar,chord in enumerate(progression):
    ...
    if bridge:
        ...
        for step,vol in [(0,34)]:note(bar,step,2,CHORD[chord],48,vol if not intro else vol-5)
...
patterns=ROWS//64
header=b'B-SKYWARD'.ljust(20,b'\0')
...
(OUT/'B-Skyward.mod').write_bytes(module)
...
with wave.open(str(OUT/'B-Skyward-preview.wav'),'wb') as w:
    w.setparams((2,2,44100,0,'NONE','not compressed'));w.writeframes(np.rint(x).astype('<i2').tobytes())
report={'title':'Sparkpaw — B-Skyward','status':'original title candidate; user listening pending','bpm':BPM,'speed':SPEED,'channels':4,'patterns':patterns,'bars':BARS,'render_seconds':seconds,'module_bytes':len(module),'sample_bytes':sum(len(d) for _,d,_ in samples),'module_sha256':hashlib.sha256(module).hexdigest(),'peak_before_master':int(abs(pcm.astype(np.int32)).max()),'preview_gain':gain,'reference_samples_used':False,'runtime_integrated':False,'revision':'B-Skyward: extended original melody and four-channel arrangement','render_engine':'micromod, actual MOD playback','samples':[{'name':n,'bytes':len(d),'volume':v} for n,d,v in samples]}
