...
### 5 September 2026 — real-Amiga campaign acceptance and low-Chip audit

User reports successful latest-alpha (0.7.0-alpha.2) ordinary HD and campaign
WHDLoad testing on real Amiga. Separately, HD starts and plays Level 1 with
about 1.45 MB free Chip RAM but crashes on Stormrail. Exact failure stage,
free/largest bytes and Guru/exit behaviour were not supplied. Physical ADF/Gotek,
Pocket and intermittent HUD evidence remain separate open gates.

Read-only source/header audit finds 256-row Stormrail world assets versus
208-row Level-1 assets and a 208-row renderer, plus two height-matched guarded
rear copies. A bounded height correction offers approximately 160 KiB Chip
potential before unused Level-1 cache selection; no measured savings or fix
is claimed. The Core cache alone still requests 34,560 Chip bytes in Stormrail.
Full Level-1 renderer/audio cleanup already precedes Continue loading. See
sparkpaw/docs/STORMRAIL_CHIP_RAM_AUDIT.md for safeguards and validation order.
Documentation only: no source/assets, package, version, emulator, commit or
push changes. Performance work remains parked.

Follow-up in the same session: the HD failure occurs immediately during
Stormrail loading as black output with top-edge flicker, like the earlier
low-Chip startup problem. Switching Workbench to two colours leaves 1.8+ MB
Chip free and ordinary HD including Stormrail works. WHDLoad runs Stormrail
from the approximately 1.45 MB-free Workbench setup too. This strengthens the
Chip-capacity diagnosis without identifying the exact allocation or claiming
identical HD/WHDLoad runtime memory layouts. No runtime changes made.


### 5 September 2026 — Stormrail Chip candidate 1 HD ZIP

On explicit user go, reduced only generated Stormrail front/rear/flight-rear
height from 256 to the renderer's 208 gameplay rows. Compared each plane with
the protected alpha.2 release: all retained bytes/palettes identical, all removed
rows zero. Removes 121,728 source bytes plus height-matched guarded rear storage
for approximately 160 KiB expected Chip saving. HD compilation reproduces the
accepted executable exactly. Full host suite and new visible-byte/header guards
pass; no runtime code, artwork, audio, replay or transition changes.

Staged through stage_hd_test.py and delivered only dist/Storm-Chip1-HD.zip,
867,513 bytes, 50 files (executable, ReadMe, 48 assets). ZIP contents/CRC and
102 unchanged release-file hashes verified. Internal staging moved to build.
No FS-UAE, additional package, release, commit or push. Hardware acceptance at
1.45 MB free Chip remains pending; follow-up caches/failure handling deferred.


### 5 September 2026 — offline storage/memory/loading follow-up

While the user tests Chip1, researched further savings without changing it.
STORAGE_MEMORY_LOADING_AUDIT.md records 69,720 requested Chip bytes in unused
Stormrail Level-1 caches and 187,240 avoidable graphics read bytes; a mostly
empty departure foreground offers approximately 292.5 KiB with bounded 512px
source semantics, requiring renderer safeguards. Existing codec round trips
project 48 raw assets from 2,892,290 to 1,006,837 bytes; this is installed HD
payload opportunity, not ZIP/ADF savings or native timing. Preserved audio
reuse, 512-byte HD safety, intentional title/charging holds and replay lifetime.
Chip1 ZIP hash remains c3859e7a9bf23a42dc7d8f9efbdcf05b903cc6cf61e80424563d3572bbb141b6.
No further source/package/FS-UAE/release/commit changes.


### 5 September 2026 — Chip2 skips unused Stormrail source/cache work

User says go on further savings, excluding disk compression. One isolated
follow-up skips beetle/Strider/Core/extra-life source loads plus six unused
Level-1 cache families during Stormrail. Saves 69,720 requested Chip bytes and
187,240 file bytes per load beyond Chip1; no measured native seconds claimed.
Explicit water consumer guard, unchanged shared audio/player/HUD and null-safe
cleanup preserve departure/replay lifetimes. No large-world/pointer refactor.

Actual C selection/failure tests under ASan/UBSan, full host suite and native HD
build pass. Sole candidate Storm-Chip2-HD.zip is 867,613 bytes, CRC/readback-
verified with 48 Chip1-identical assets; all 102 baseline release files match.
Chip1 archived intact. Native direct Stormrail, Continue, replay and return to
Level 1 remain required. No emulator/release/other format/commit/push.


### 5 September 2026 — Stormbound original title-music candidate

User requests a faster, action/story-appropriate Sparkpaw title tune after the
Storm Light sketch, citing supplied Turrican II title video and ThunderCats/
Marvin pace; explicitly skips the ThunderCats skill. Preserved and catalogued
the new MOV as Unassigned-reference-turrican2-title-music with its matching
TXT; frame inspection and in-memory signal analysis are not aural review.

Created music/experiments/stormbound: original 144-BPM, four-channel ProTracker
MOD, 48 bars / ~80 seconds, sixteen newly synthesized samples. Hero melody,
syncopated bass, sampled harmony, drum fills and contrasting clockwork middle.
Actual MOD replay via existing micromod produces WAV/MP3 previews. Structural,
duration and unsaturated-PCM checks pass; local ffmpeg lacks MOD input support,
so only micromod playback is claimed. User listening pending, no game integration,
new Chip budget promise, emulator, package, release or commit/push.


### 5 September 2026 — Stormbound smoother listening revision

User rejects the first tune as choppy and insufficiently melodic/rhythmic,
asks for smoother/tighter flow. Preserved v1 and authored stormbound-smooth:
longer thematic phrases, no lead C00 cuts, smoother sustaining lead/chords,
even eighth-note bass, consistent backbeat and fewer fills at the same 144 BPM.
Actual four-channel MOD renders to WAV/MP3; structural/peak/duration checks
pass. Larger sample budget is recorded, not integrated into gameplay. User
listening remains pending. No code/package/release/commit changes.


### 5 September 2026 — three Stormbound musical directions

User prefers Smooth and requests more epic long melodies, pumping beat and
classic Amiga identity. Created three independent listening variations:
Stormdrive 152 BPM, Skyward 144 BPM, Paula Power 150 BPM. Composite kick/snare
samples preserve true four-channel limits; longer melodic arcs and sustaining
samples preserve flow, with zero lead hard cuts. Actual MOD renders, durations,
hashes and unclipped PCM checked. Previews share peak target; no aural/model or
hardware quality claim. Earlier tunes and all game packages preserved. Expanded
sample budgets remain listening experiments pending user preference/integration.


### 5 September 2026 — faster Stormbound synth variations

User requests further Turrican/Marvin-inspired pace and classic synth timbres.
Created Voltage Run (168 BPM), Neon Sky (160 BPM), Turbo Shards (176 BPM) under
music/experiments/stormbound-synth. Original harmonic synth samples, detuned
lead, stronger bass saturation and retained flowing phrases; no reference
sample/melody copying. Actual MOD renders, hashes, durations and MP3 decoding
verified. Reference analysis is not aural listening; musical acceptance remains
with the user. No runtime integration, package changes, release or commit/push.

### 6 September 2026 — Chip2 HD accepted as development baseline
...
Normal make is current; executable and all 48 runtime assets match the accepted
ZIP byte-for-byte and ZIP CRC passes. No additional code change is needed to
integrate: the optimizations already use the normal build paths. Documentation
and lessons synchronized. No emulator, packaging, release, commit or push.

### 6 September 2026 — Neon Sky title integration and three intro studies

User selects Neon Sky (160 BPM) for large title through READY; gameplay stays
SFX-only. Added LSP-based presentation music with OS/polled display ownership,
Fast score and 142,796-byte Chip bank, freed after READY fade. Full host suite,
actual lifecycle/failure tests and native build pass. Sole staged test drawer
Title-Music-HD has 50 verified assets; all 102 release files remain identical.
User explicitly orders FS-UAE/030, then accepted 020/performance, then A1200.
No emulator/release/other media/commit/push. Hardware acceptance remains pending.

Inspected production intro art and eleven text passages (~61 seconds). Created
three original 60.952-second MOD/WAV/MP3 intro cues with spacious synth melodies,
slow pulse and narrative changes. Listening choice pending, no intro integration.
See sparkpaw/docs/TITLE_AND_INTRO_MUSIC.md for timing, sources and exact boundaries.

### 6 September 2026 — three livelier A Shard of Hope revisions

User prefers the third intro cue and asks for a happier first scene, dark middle,
hopeful ending, rhythmic variety and tempo changes. Created Morning Sparks,
Clockwork Dawn and Skybound, each ~60–61 seconds, with distinct grooves and
encoded per-scene Fxx tempos. Four-channel MOD replay, durations, hashes, tempo
events, peak checks and MP3 decoding verified; user listening pending. Preserved
all earlier music and the active title-music game test without runtime changes.
...
### 7 September 2026 — Hero Drive intro integration and dist archive
...
### 7 September 2026 — module replay alternatives audit

User tentatively perceives music disturbance/skipping and asks about the player.
Inspected actual LSP fixed50/VBlank-to-polled READY integration and researched
upstream LSP CIA and ptplayer 6.4. Recorded timing quantization, possible missed
READY ticks and DMA busy-wait/PAL-wrap concern separately from unconfirmed
native symptoms. Recommend locate phase/timestamp and compare original/LSP/native
passage before bounded timer/player changes. No runtime or test-build changes.
See sparkpaw/docs/MUSIC_REPLAY_AUDIT.md.
...
## 2026-09-07 — Release 0.7.0-alpha.3
